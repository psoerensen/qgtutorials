...
Polygenic risk scoring combines information from large numbers of markers across the genome (hundreds to millions) to give a single numerical score for an individual’s relative risk for developing a specific disease on the basis of the DNA variants they have inherited. \textcolor{red}{Detalje, men overvej om du istedet vil kalde det for Genomic (risk) scores da ordlyden passer bedre på kvantitative traits.}
...
* Ability to identify clinically meaningful increases in disease risk predictions \textcolor{red}{Forstår ikke denne sætning :) Clinical hvad?}
...
\textcolor{blue}{\Huge \textbf{!}} \textcolor{blue}{{\bf Task for you\\}}1. Use the histogram and boxplot functions to visualize the distribution the trait/covariables.
...
\textcolor{blue}{\Huge \textbf{!}} \textcolor{blue}{{\bf Task for you\\}}1. What is a GWAS\newline 2. What are GWAS summary statistics?
...
\textcolor{blue}{\Huge \textbf{!}} \textcolor{blue}{{\bf Task for you\\}}1. In the context of a GWAS, why is information about LD important?\newline 2. Plot the average LD decay for a subset of genetic variants \newline(\textit{Hint}: $LD <- getLD(Glist=Glist, chr=1, rsids=Glist\$rsidsLD[[1]][5000:5010])$)
...
In the Bayes N approach the prior for each marker effect follows a priori a normal distribution with a variance $\sigma_b^2$ which is constant across markers:
...
paN <- acc(yobs=y[valid], ypred=prs[valid,], typeoftrait="binary")
```
